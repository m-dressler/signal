import { isSignal, OrSignal } from "@signal/core";

/**
 * Parameters for {@link createElement}
 *
 * Allows you to specify initial classes, attribute, styles, all with potentially Signal values
 * which will automatically get reflected on the element
 */
type CreateParams<
  T extends HTMLElementTagNameMap[keyof HTMLElementTagNameMap]
> = {
  -readonly [K in keyof T]?: OrSignal<K extends "style" ? Partial<T[K]> : T[K]>;
};
